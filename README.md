# Netflix Landing Page Clone (2022)

> **Note:** This is a static frontend clone of the Netflix 2022 Landing Page created for educational purposes.

## 📄 Project Overview

This project is a responsive recreation of the Netflix landing page using **Bootstrap** for layout and **CSS** for branding. The goal was to demonstrate how to effectively use a CSS framework to build pixel-perfect, mobile-first web pages.

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-563D7C?style=for-the-badge&logo=bootstrap&logoColor=white)

### 🌟 Key Features

* **Bootstrap Grid System:** Uses the Bootstrap container and grid system for a fully responsive layout across mobile, tablet, and desktop.
* **Bootstrap Components:** Utilizes the **Bootstrap Accordion** for the FAQ section and **Input Groups** for the email signup form.
* **Custom Styling:** Overrides default Bootstrap styles to match Netflix’s specific color palette (`#e50914`) and typography.
* **Background Overlay:** Implements the signature Netflix dark gradient overlay using CSS.

## 🛠️ Tech Stack

* **Framework:** [Bootstrap 5](https://getbootstrap.com/) (CDN)
* **Structure:** HTML5
* **Styling:** CSS3 (Custom Styles + Bootstrap Utilities)
* **Icons:** FontAwesome / Bootstrap Icons
* **Editor:** VS Code

## 🚀 Live Demo

[**View Live Demo Here**](#) *(Replace this text with your GitHub Pages or Netlify link)*

## 📸 Screenshots

| Desktop View | Mobile View |
|:---:|:---:|
| ![Desktop](./assets/desktop-preview.png) | ![Mobile](./assets/mobile-preview.png) |

## 💻 Installation & Setup

Since this project uses static HTML and a CDN for Bootstrap, no package installation is required.

1.  **Clone the repository**
    ```bash
    git clone [https://github.com/chibsaabji/netflix.git](https://github.com/chibsaabji/netflix.git)
    ```

2.  **Navigate to the folder**
    ```bash
    cd netflix-clone-2022
    ```

3.  **Run the Project**
    * Simply double-click `index.html` to open it in your browser.
    * **OR** use the "Live Server" extension in VS Code for hot-reloading.

## 🤝 Contributing

Contributions are welcome! If you find any responsive issues, feel free to open a pull request.

## 📝 License

This project is open source and available under the [MIT License](LICENSE).

---

**Author:** Vanshveer Chib  
**GitHub:** [Chibsaabji](https://github.com/chibsaabji)
